...
function create_default_menu_items()
	return {
		...
		{title = ulang._cm_navigation, items = {
			...
			{title = "精确跳转", value = 'script-binding uosc/precise-seek'},
		},},
...
-- 精确跳转功能
function open_precise_seek_menu()
	if Menu:is_open('precise-seek') then
		Menu:close()
		return
	end
	...
	local menu = Menu:open({
		type = 'precise-seek',
		title = '精确跳转',
		items = {
			{
				title = '当前时间: ' .. format_time(current_time),
				selectable = false,
				muted = true
			},
			{
				title = '总时长: ' .. format_time(duration),
				selectable = false,
				muted = true
			},
			{
		...
		if event.type == 'activate' and event.value == 'input' then
			local input = mp.get_osd_size()
			mp.commandv('osd-overlay', 'input', '输入时间 (HH:MM:SS):', function(text)
				if text then
					local target_time = parse_time(text)
					if target_time and target_time >= 0 and target_time <= duration then
						mp.commandv('seek', tostring(target_time), 'absolute')
					else
						mp.commandv('show-text', '无效的时间格式或超出范围', 3000)
					end
				end
			end)
		end
...
-- 绑定精确跳转命令
bind_command('precise-seek', open_precise_seek_menu)
